...
function KpiCard({ label, value, sub, color }: { label: string; value: string; sub?: string; color?: string }) {
  return (
    <div className="kpi-card animate-card">
      ...
      <div className="kpi-value" style={{ color: color ?? 'var(--amber)' }}>{value}</div>
  ...
  return (
    <div className="caisse-view">
      ...
      <div className="kpi-row">
        <KpiCard label="Recette du jour" value={fmtMoney(revenue)} sub={`${sessionsCount} session${sessionsCount !== 1 ? 's' : ''}`} color="var(--amber)" />
        <KpiCard label="Clients identifiés" value={String(uniqueClients)} sub="avec ticket" color="var(--green)" />
        <KpiCard label="Taux remplissage" value={fillPct + '%'} sub={`sur 12h × ${postes.length} postes`} color="var(--blue)" />
        <KpiCard label="Boissons / extras" value={fmtMoney(extrasRevenue)} sub={`${extrasCount} vendue${extrasCount !== 1 ? 's' : ''}`} color="var(--amber)" />
      </div>
      ...
      {todaySessions.length === 0 ? (
        <EmptyState
          ...
          body="Les sessions encaissées apparaîtront ici au fil de la journée"
        />
      ) : (
