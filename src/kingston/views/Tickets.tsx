...
import { generateTicketCode, TICKET_VALID_MS, isTicketValid, ticketStatus, fmtDate, fmtDateTime, fmtMoney, fmtDuration, fmtMs } from '../lib/helpers';
import { showToast } from '../components/Toast';
import { playTicketScan } from '../lib/audio';
...
export function TicketsView() {
  ...
  function handleCreate() {
    ...
    if (!n || !p) { showToast('Nom et prénom requis', '⚠️', '#E8A33D'); return; }
    if (!a || a < 1 || a > 120) { showToast('Âge invalide', '⚠️', '#E8A33D'); return; }
    setCreating(true);
    ...
    showToast(`Ticket créé — code ${code}`, '🎫', '#E8A33D');
  ...
  return (
    <div className="tickets-view">
      ...
      <div className="search-bar">
        ...
        <button className="scan-trigger-btn" onClick={() => setShowScanner(true)} title="Scanner un QR code">📷</button>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="empty-state">
          ...
          <p>{search ? 'Aucun ticket trouvé' : 'Aucun ticket pour l\'instant — crée le premier !'}</p>
        </div>
      ) : (
