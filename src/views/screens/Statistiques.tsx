...
import { fmtMoney, fmtDuration, dayKey, getWeekStart, getMonthStart } from '../../lib-app/helpers';
...
function StatCard({ label, value, sub, icon, color }: { label: string; value: string; sub?: string; icon: string; color?: string }) {
  return (
    <div className="stat-card animate-card">
      ...
      <div className="stat-value" style={{ color: color ?? 'var(--amber)' }}>{value}</div>
  ...
  return (
    <div className="stats-view">
      ...
      <div className="stats-period-grid">
        <div className="period-block">
          ...
          <div className="period-stats">
            <div className="period-kpi"><span className="pk-v" style={{ color: 'var(--amber)' }}>{fmtMoney(stats.todayRevenue)}</span><span className="pk-l">recette</span></div>
            <div className="period-kpi"><span className="pk-v" style={{ color: 'var(--green)' }}>{stats.todaySessions}</span><span className="pk-l">sessions</span></div>
            <div className="period-kpi"><span className="pk-v" style={{ color: 'var(--blue)' }}>{stats.todayClients}</span><span className="pk-l">clients</span></div>
          </div>
        </div>
        <div className="period-block">
          <div className="period-label">📆 Cette semaine</div>
          <div className="period-stats">
            <div className="period-kpi"><span className="pk-v" style={{ color: 'var(--amber)' }}>{fmtMoney(stats.weekRevenue)}</span><span className="pk-l">recette</span></div>
            <div className="period-kpi"><span className="pk-v" style={{ color: 'var(--green)' }}>{stats.weekSessions}</span><span className="pk-l">sessions</span></div>
            <div className="period-kpi"><span className="pk-v" style={{ color: 'var(--blue)' }}>{stats.weekClients}</span><span className="pk-l">clients</span></div>
          </div>
        </div>
        <div className="period-block">
          <div className="period-label">🗓 Ce mois</div>
          <div className="period-stats">
            <div className="period-kpi"><span className="pk-v" style={{ color: 'var(--amber)' }}>{fmtMoney(stats.monthRevenue)}</span><span className="pk-l">recette</span></div>
            <div className="period-kpi"><span className="pk-v" style={{ color: 'var(--green)' }}>{stats.monthSessions}</span><span className="pk-l">sessions</span></div>
            <div className="period-kpi"><span className="pk-v" style={{ color: 'var(--blue)' }}>{stats.monthClients}</span><span className="pk-l">clients</span></div>
          </div>
        </div>
        <div className="period-block all-time">
          <div className="period-label">🏆 Tout temps</div>
          <div className="period-stats">
            <div className="period-kpi"><span className="pk-v" style={{ color: 'var(--amber)' }}>{fmtMoney(stats.totalRevenue)}</span><span className="pk-l">total</span></div>
            <div className="period-kpi"><span className="pk-v" style={{ color: 'var(--green)' }}>{stats.totalSessions}</span><span className="pk-l">sessions</span></div>
            <div className="period-kpi"><span className="pk-v" style={{ color: 'var(--blue)' }}>{stats.totalTickets}</span><span className="pk-l">tickets</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="stat-cards-grid">
        <StatCard icon="📊" label="Moy. sessions/jour" value={String(stats.avgSessionsPerDay)} sub="(7 derniers jours)" color="var(--blue)" />
        <StatCard icon="🥤" label="Sessions avec boisson" value={`${stats.drinkPct}%`} sub="des sessions" color="var(--amber)" />
        <StatCard icon="🎫" label="Tickets valides" value={String(stats.validTickets)} sub={`/ ${stats.totalTickets} total`} color="var(--green)" />
        <StatCard icon="📱" label="Mobile Money" value={`${stats.mobileMoneyPct}%`} sub={`Airtel ×${stats.airtelCount} · MTN ×${stats.mtnCount}`} color="var(--kg-violet)" />
        <StatCard icon="⏱" label="Postes actifs" value={String(postes.filter(p => p.status === 'busy').length)} sub={`/ ${postes.length} postes`} color="var(--red)" />
      </div>
              ...
              <tbody>
                {stats.topPostes.map(p => (
                  <tr key={p.name}>
                    <td>{p.name}</td>
                    <td>{p.sessions}</td>
                    <td style={{ color: 'var(--green)' }}>{fmtMoney(p.revenue)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
