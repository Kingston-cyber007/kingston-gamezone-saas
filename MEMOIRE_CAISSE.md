...
| A.5b | Refonte totale — **A.3 — Push migrations Supabase côté DB** (étape externe côté Yannick, documentée a posteriori). Cible : projet Supabase `ybqsfyufnpmhlhdtwkon` (Free tier, dashboard OK). Préparation : `supabase/config.toml` corrigé en A.2b.4 (project_id `hxxedbccrdyshjlyfhpv` → `ybqsfyufnpmhlhdtwkon`), CLI v2.110.0 installé, doc `supabase/README_DEPLOY.md` (commit 563597e) servant de mode opératoire. **Exécution (côté Yannick le 02/08/2026)** : (1) `supabase login` → token `cli_DELL@Yannick-kingston007_1785692837` créé ; (2) `supabase link --project-ref ybqsfyufnpmhlhdtwkon` → "Finished supabase link." ; (3) `supabase migration list` → diagnostic majeur : **4 migrations déjà en remote** (20260723171443, 20260724002457, 20260725113348, 20260726150000), **1 seule nouvelle à pousser** (20260731150000_5c1) ; la migration 5A.1 `platform_admins` avait donc été appliquée lors d'une session antérieure (cf. RT.H.7 dans le journal historique) ; (4) `supabase db push` → "Applying migration 20260731150000_5c1_platform_admins_view_profiles.sql... Finished supabase db push." **Vérifications brutes (Dashboard → SQL Editor)** : (a) `SELECT policyname FROM pg_policies WHERE tablename = 'profiles' AND policyname LIKE '%Platform%';` → **1 ligne** : `Platform admins can view all profiles` ✓ ; (b) `SELECT email, notes, created_at FROM platform_admins;` → **2 lignes** : `ykingston007@gmail.com | Admin fondateur — bootstrap initial | 2026-07-31 04:23:25` (seed initial 5A.1) + `admin@kingstongamezone.com | compte test admin | 2026-07-31 04:31:18` (ajout manuel RT.H.1) ✓ ; (c) `npx tsc --noEmit` exit 0 (sanity check côté code, les 6 calls Supabase à `platform_admins` sont alignés avec les policies). **Effet** : (1) dette #11 (policy view profiles) **fermée** côté DB ; `TenantUsersModal` peut désormais afficher les vrais emails pour les platform_admins lors du prochain rendu ; (2) dette #2 (table platform_admins non appliquée) **fermée** côté DB — la migration était déjà poussée, on l'a découvert au `migration list`. **Aucun secret copié** (token CLI dans l'historique bash de Yannick, jamais committé, `.env` reste sur disque local). **Non poussé en remote** : `git push` des 11 commits atomiques — c'est externe (décision de Yannick quand il veut). **Validation Yannick :** 02/08/2026 (« A.3 close »). | ✅ Validé | 02/08/2026 |
...
| ND1-11 | Session 03/08/2026 — Audit global + 11 fixes (ND1-ND11) + ouverture CHANTIER 1 | ✅ Constaté sur disque (non committé avant le 12/08, trou de gouvernance comblé) | 03/08/2026 |
| CH.1 | CHANTIER 1 — Fondations (profiles étendus, reservations, consoles, staff_applications, inscription client/staff) | 🟡 En cours (migration + edge function + auth + platform codés, push DB externe non exécuté) | 03/08→10/08/2026 |
| CH.1B.3 | CHANTIER 1B.3 — Edge Function `check-email-jetable` (anti-jetable, mailchecker serveur) | ✅ Constaté sur disque (déploiement edge externe requis) | 07/08/2026 |
...
**Synthèse de la session 03/08→12/08 (constatée sur disque, documentée a posteriori — cf. journal) :**
- **ND1-ND11** : audit global (5 agents) → XSS `escapeHtml`, RBAC `/client`, validation `/platform`, PII chiffrées AES-GCM + anonymisation 7j (`secure-storage.ts`), guards `merge` Zustand, bascule npm→pnpm, `SECURITY.md`.
- **CHANTIER 1 — Fondations** : migration `20260803113500` (profiles étendus + `reservations` + `consoles` + `staff_applications` + statut `reserved`), edge function anti-jetable, formulaire inscription client + candidature staff, écran approbation `/platform`.
- **CHANTIER 3 — Suppression de compte** : migration `20260808220000` (soft-delete 30j + purge pg_cron), FKs `profiles`, seed fixtures, edge functions `delete-own-account` + `restore-account-admin`, `DoubleConfirmModal` (saisie mot-clé).
- **RLS 11/08** : 3 migrations de durcissement `tenants` (42501 anon → policies `TO authenticated`).
- **CHANTIER 2 — Paiement CinetPay** : migration `20260812000000` (RPC `compute_reservation_price`, `payments`, `idempotency_keys`, enum `carte`), edge function `create-payment-intent`.
...
- **Non utilisé en l'état** par la branche SaaS Lovable (archive : `Gaming-Ticket-Manager.zip`).
...
**Prochaine étape chantier** : trancher ce qu'on fait des travaux non committés de la session 03/08→12/08 (déjà committés au 12/08), puis décider la suite : push DB/edge, isolation RLS (dette #5, phase C), licences (dette #4), ou consolidation offline-first (étape 6).
...
| 30/07/2026 | **RT.A.1 — Nettoyage structurel** — suppression de `src/kingston/` (13 fichiers orphelins : components, views, store, i18n, lib, kingston.css), suppression de `src/assets/kingston-logo.png.asset.json`, déplacement `src/kingston/kingston.css` (44 Ko, 1700+ lignes) → `src/views/theme.css`, nettoyage de `logoAsset` dans 4 routes hors périmètre (`auth.tsx`, `index.tsx`, `client/route.tsx`, `platform/route.tsx`) — toutes utilisent désormais `@/assets/logo.png` directement, plus de `.asset.json`. Modification de l'import CSS dans `app/route.tsx` : `@/kingston/kingston.css` → `@/views/theme.css`. **Vérifications brutes** : `npx tsc --noEmit` → exit 0, **80 erreurs shadcn** (vs 84 avant RT.A.1, **-4 erreurs** récupérées en bonus : le nettoyage de `src/kingston/*` a libéré certains types chaînés dans `src/components/ui/*.tsx`) ; `npx vite build` → exit 0 ; `grep -rn "@/kingston/" src/` → 0 ; `grep -rn "logoAsset" src/` → 0 ; `grep -rn "kingston-logo.png.asset.json" src/` → 0 ; `grep -rn "kingston.css" src/` → 0 ; `ls src/kingston` → n'existe plus. **État final `src/views/`** : `components/`, `hooks/`, `screens/`, `theme.css` (structure cible atteinte). **État final `src/assets/`** : uniquement `logo.png` (96 Ko). **Écarts par rapport au plan initial** : aucun. **Re-confirmation anti-flash étape 2** : `app/route.tsx` reste fonctionnel avec le CSS déplacé → `LoadingScreen` + `SplashScreen` + `PowerCutOverlay` toujours alimentés par leurs classes CSS. **Re-confirmation Logo 3B** : les 5 routes qui utilisent le logo consomment désormais `@/assets/logo.png` directement, sans dépendance à `kingston-logo.png.asset.json` (mort). **Validation par Yannick ("OK")** — tableau de suivi mis à jour : RT.A.1 = ✅. Prochain déclenchement : RT.A.2 (vérification finale). |
| 30/07/2026 | **RT.A.2 — Vérification finale** — étape purement diagnostique, aucune modification appliquée. Re-exécution des vérifications brutes sur l'état frais du dépôt : `npx tsc --noEmit` → **80 erreurs** (toutes dans `src/components/ui/*.tsx`, 41 fichiers concernés, dette pré-existante documentée 27/07 — aucune dans le code applicatif) ; `npx vite build` → **exit 0** — client 1.43s (527 modules) + SSR 1.21s (108 modules) + Nitro 632ms (542 modules). Invariants structurels RT.A tous au vert : `src/kingston/` Glob → 0 fichier ; `src/views/theme.css` présent (44 768 octets) ; `src/views/` = `components/`, `hooks/`, `screens/`, `theme.css` (structure cible) ; `src/assets/` = uniquement `logo.png` (96 740 octets) ; 0 occurrence `logoAsset` / `kingston-logo.png.asset.json` / `kingston.css` / `@/kingston/` / `src/kingston` dans `src/`. Invariants 5 routes logo : `auth.tsx`, `index.tsx`, `client/route.tsx`, `platform/route.tsx`, `app/route.tsx` importent tous `@/assets/logo.png` (les 5 routes filles `app/{caisse,reglages,salle,stats,tickets}.tsx` héritent via le parent) ; `app/route.tsx` importe `@/views/theme.css` (les autres routes n'en ont pas besoin — CSS arrive via `__root.tsx` ou le parent). **Découverte d'état hors périmètre** : incohérence `package.json` ↔ `node_modules/` (package.json liste 59 deps dont `@radix-ui/*` ×24, `class-variance-authority`, `recharts`, `zod`, etc. ; node_modules n'en contient que ~19 — cohérent avec l'étape 4bis). **Conclusion** : l'étape 4bis a bien appliqué ses retraits dans `node_modules/` mais `package.json` n'a pas été committé. **Aucune régression** pour le build (vite exit 0, tsc exit 0). **Décision** : ne pas traiter maintenant (hors périmètre RT.A, règle 1) — à re-synchroniser dans un sous-bloc dédié si pertinent. **Synthèse RT.A** : refonte structurelle totalement et solidement terminée — (1) 11 imports runtime réécrits (RT.A.0) ; (2) 14 fichiers structurellement orphelins supprimés (RT.A.1) ; (3) CSS de marque déplacé dans la nouvelle structure (RT.A.1) ; (4) erreurs tsc réduites de 84 → 80 (bonus nettoyage) ; (5) 0 résidu, 0 dépendance ajoutée, 0 régression applicative (RT.A.2). **Validation par Yannick ("OK — j'inscris au journal et on passe à la suite")**. RT.A clôturé proprement. |
| 30/07/2026 | **RT.B.1 — Sécurisation effective** — `.env` retiré du suivi git via `git rm --cached .env` (fichier préservé sur disque, 367 octets, intact) ; `.gitignore` complété avec `.env`, `.env.*`, exception `!.env.example`. **Contexte** : l'étape 1 du plan initial (`NOUVEAU_DEPART_prompt.md`) était marquée `✅ Validée` au 26/07/2026 mais **n'avait jamais réellement été appliquée** dans le code (`.env` toujours tracké, `.gitignore` ne contenait pas `.env`). Le journal était en avance sur le code. RT.B.1 est le **premier sous-bloc** d'une série (RT.B) ouverte après audit contradictoire complet du 30/07 : (a) réparation des étapes passées prétendument validées mais non-effectives (4A, 4bis, 4ter, 3C, 3D, 3E, 5B.3) ; (b) refonte visuelle et structurelle complète exigée par `REFONTE_TOTALE_cahier_des_charges.md` §1 (HTML, CSS, React hérités de Lovable doivent être cassés et reconstruits). **Vérifications brutes** : `git ls-files .env` → vide ; `ls .env` → 367 octets présent ; `git status --short .env` → `D .env` (deleted from tracking) ; `.gitignore` lignes 35-37 contiennent `.env`, `.env.*`, `!.env.example` ; aucune dépendance ajoutée. **Écarts par rapport au plan initial** : oui — l'étape 1 avait été validée sans exécution réelle (le journal R17 du 26/07 prétendait `.env` retiré mais ne l'était pas). **Validation par Yannick ("OK — inscrire RT.B.1, puis RT.B.2")**. Prochain déclenchement : RT.B.2 (purge effective `src/components/ui/` (46 fichiers) + `src/hooks/use-mobile.tsx` + `components.json`). |
| 30/07/2026 | **RT.B.2 — Purge effective shadcn** — suppression de `src/components/ui/` (46 fichiers : accordion, alert-dialog, alert, aspect-ratio, avatar, badge, breadcrumb, button, calendar, card, carousel, chart, checkbox, collapsible, command, context-menu, dialog, drawer, dropdown-menu, form, hover-card, input-otp, input, label, menubar, navigation-menu, pagination, popover, progress, radio-group, resizable, scroll-area, select, separator, sheet, sidebar, skeleton, slider, sonner, switch, table, tabs, textarea, toggle-group, toggle, tooltip), `src/hooks/use-mobile.tsx` (shadcn utilitaire), `src/hooks/` (dossier vide résiduel), `src/components/` (dossier vide résiduel), `components.json` (config shadcn). **Contexte** : l'étape 4A du plan initial prétendait avoir supprimé ces éléments le 26/07/2026 mais **rien n'avait réellement été supprimé** (46 fichiers shadcn + components.json toujours présents au moment de RT.B). Le journal était en avance sur le code. RT.B.2 applique **réellement** cette suppression. **Vérifications brutes** : `npx tsc --noEmit` → **0 erreur** (vs 80 erreurs shadcn avant RT.B.2, **toute la dette tsc provenait bien de ces fichiers**) ; `npx vite build` → exit 0 (client 8.07s, SSR 6.60s, Nitro 3.95s, 527+108+542 modules) ; `src/components/` GONE ; `src/hooks/` GONE ; `components.json` GONE ; aucun fichier applicatif modifié ; aucun import applicatif `@/components/ui/*` à corriger (0 occurrence). **Écarts par rapport au plan initial** : oui — l'étape 4A validée au 26/07 n'avait pas réellement été appliquée. **Validation par Yannick ("OK — inscrire RT.B.2, puis RT.B.3")**. Prochain déclenchement : RT.B.3 (purge des 37 deps mortes dans `package.json` + réinstallation via `npm install`). |
| 30/07/2026 | **RT.B.3 — Purge effective deps mortes** — retrait de 39 dépendances du bloc `dependencies` de `package.json` : les 24 packages `@radix-ui/*` (`react-accordion`, `react-alert-dialog`, `react-aspect-ratio`, `react-avatar`, `react-checkbox`, `react-collapsible`, `react-context-menu`, `react-dialog`, `react-dropdown-menu`, `react-hover-card`, `react-label`, `react-menubar`, `react-navigation-menu`, `react-popover`, `react-progress`, `react-radio-group`, `react-scroll-area`, `react-select`, `react-separator`, `react-slider`, `react-slot`, `react-switch`, `react-tabs`, `react-toggle`, `react-toggle-group`, `react-tooltip`) + 14 autres (`@hookform/resolvers`, `class-variance-authority`, `cmdk`, `date-fns`, `embla-carousel-react`, `input-otp`, `react-day-picker`, `react-hook-form`, `react-resizable-panels`, `recharts`, `sonner`, `vaul`, `zod`, `tw-animate-css`). Total `dependencies` : **59 → 20** (37 retirées, écart mineur avec le plan initial qui visait ~19 — `framer-motion` et `lucide-react` confirmés à 0 usage dans `src/` mais **conservés par prudence** pour validation ultérieure). **Contexte** : l'étape 4bis du plan initial prétendait avoir synchronisé `package.json` ↔ `node_modules` au 26/07/2026 mais **rien n'avait réellement été retiré** du manifeste (`@radix-ui/*` ×24 toujours présents, 0 retiré). Le journal était en avance sur le code (cf. RT.A.2 § « Découverte d'état hors périmètre »). RT.B.3 applique **réellement** ce retrait. **Vérifications brutes** : `npm install --no-audit --no-fund` → exit 0 (« up to date in 7s », aucune erreur EBADENGINE bloquante — juste warning `@zxing/library` engine) ; `npx tsc --noEmit` → **0 erreur** (exit 0) ; `npx vite build` → exit 0 (Nitro généré sans erreur) ; `node_modules/@tanstack/` contient `react-start`, `react-router`, `react-query` ; `node_modules/tw-animate-css/dist/` contient `tw-animate.css` + `tw-animate-prefix.css` ; `node -e` confirme 18/20 deps résolvables (les 2 « missing » `@tanstack/react-start` et `tw-animate-css` sont en réalité présents — faux positif de `require.resolve` sur leur `exports` field). **Écarts par rapport au plan initial** : oui — l'étape 4bis validée au 26/07 n'avait pas réellement été appliquée (package.json non nettoyé). 3 deps en attente de décision : `framer-motion`, `lucide-react`, `tw-animate-css` (ce dernier sera retiré en RT.B.5 lors de la purge de `src/styles.css` qui l'importe). **Validation par Yannick (« ok »)**. Prochain déclenchement : RT.B.4 (réconciliation lockfiles — retrait `bun.lock` + `bunfig.toml` au profit de `package-lock.json`).
| 30/07/2026 | **RT.B.4 — Réconciliation lockfiles** — `bun.lock` (177 Ko) et `bunfig.toml` (470 octets) retirés du disque **et** du suivi git via `git rm --cached` (puis suppression effective). `package-lock.json` (216 Ko, 30/07 09:33, lockfileVersion 3) reste le seul lockfile du projet, marqué `M` par git (modifié par le `npm install` de RT.B.3, comportement attendu). **Contexte** : la ligne 425 du journal signalait déjà l'incohérence (deux lockfiles coexistants pour un projet qui n'utilise que `npm` selon les scripts). C'était un résidat de l'héritage Replit. **Vérifications brutes** : `ls *.lock *.toml package-lock.json` → seul `package-lock.json` listé ; `node -e "require('./package-lock.json').lockfileVersion"` → `3` (npm 7+) ; `git ls-files bun.lock bunfig.toml` → vide ; `git status --short` → `D bun.lock`, `D bunfig.toml`, `M package-lock.json` ; `.prettierignore` liste déjà `package-lock.json` (aucune modif nécessaire) ; aucun fichier hors `MEMOIRE_CAISSE.md` ne référence `bun.lock`/`bunfig.toml`/`bunfig` (la mention `bun-types-no-globals` dans `package-lock.json` est une dep transitive de `bun-types`, sans impact) ; `npx tsc --noEmit` → exit 0 ; `npx vite build` → exit 0 (Nitro généré sans erreur). **Écarts par rapport au plan initial** : aucun. **Validation par Yannick (« ok »)**. Prochain déclenchement : RT.B.5 (purge CSS dupliqués : `src/styles.css` réduit aux imports minimaux, `src/theme.css` supprimé car doublon de `src/views/theme.css`, correction import `@/theme.css` → `@/views/theme.css` dans `auth.tsx`, retrait `tw-animate-css` de `package.json`).
| 30/07/2026 | **RT.B.5 — Purge CSS dupliqués** — `src/styles.css` réduit de **144 lignes (5 559 octets) à 3 lignes (54 octets)** : ne reste que `@import "tailwindcss" source(none); @source "../src";`. Suppression de `src/theme.css` (53 603 octets, **orphelin total** : aucun import actif, aucun consommateur des variables `--kg-violet`/`--kg-cyan`/`--kg-gradient`/etc., vérifié par `grep -rn "var(--kg-" src/` qui ne renvoie que `src/theme.css` lui-même). `src/views/theme.css` (44 768 octets) reste **intact** et reste le seul CSS de marque actif (importé par `src/routes/_authenticated/app/route.tsx:3`). `tw-animate-css` retiré de `package.json` (20 → **19 deps**, chiffre exact visé par le plan initial). `npm install` → exit 0 (« removed 1 package in 3s »). **Contexte** : l'étape 4ter du plan initial prétendait avoir nettoyé ces CSS au 26/07 mais n'avait rien fait (les deux fichiers doublons coexistaient depuis, et `styles.css` contenait encore tout le boilerplate shadcn). RT.B.5 applique réellement ce nettoyage. **Vérifications brutes** : `ls src/styles.css` → 54 octets, 3 lignes ; `ls src/theme.css` → not found ; `ls src/views/theme.css` → 44 768 octets, intact ; `node -e` → 19 deps ; `node_modules/tw-animate-css` → not found ; `npx tsc --noEmit` → exit 0 ; `npx vite build` → exit 0 (Nitro généré sans erreur, malgré les classes `bg-background`/`text-foreground` encore utilisées dans NotFound/Error de `__root.tsx` qui ne sont plus définies — Tailwind tombe en fallback CSS sans crash, fix visuel prévu en RT.B.6). **Découverte annexe à signaler** : la palette Kingston (`--kg-violet`/`--kg-cyan`/`--kg-gradient`/etc.) était définie uniquement dans `src/theme.css` (supprimé) et n'a **jamais été mergée** dans `src/views/theme.css` (qui définit une palette `--amber` orangée, visiblement l'ancienne avant le rebranding Kingstone). Donc **l'identité visuelle de marque n'est pas chargée** dans l'app actuelle. C'est un autre chantier (refonte de `src/views/theme.css`) qui devra être traité séparément dans une étape RT.B.x ultérieure. **Écarts par rapport au plan initial** : oui, l'étape 4ter n'avait pas été appliquée ; par ailleurs le plan prévoyait initialement une correction d'import `@/theme.css` → `@/views/theme.css` dans `routes/auth.tsx` qui s'est révélée **non nécessaire** : `auth.tsx` importe déjà `@/views/theme.css` (cf. grep initial), et l'import `@/theme.css` n'existe nulle part dans le code (l'audit pré-RT.B.5 l'a confirmé). **Validation par Yannick (« ok »)**. Prochain déclenchement : RT.B.6 (refonte `src/routes/__root.tsx` : NotFound + Error en identité Kingston GameZone, FR, monogramme violet/cyan, thème `#7c3aed`, `data-motion`, `pendingComponent: SplashScreen`).
| 30/07/2026 | **RT.B.6 — Refonte `__root.tsx` (NotFound/Error Kingstone)** — réécriture complète de `src/routes/__root.tsx` (122 → 261 lignes) : **NotFound** avec monogramme `404` en dégradé violet→cyan via `Kg404Mark` (text-clip + drop-shadow violet), fond `radial-gradient(ellipse_at_top, rgba(124,58,237,0.18), #0a0614 60%)`, h1 « Page introuvable », paragraphe FR, bouton « Retour à l'accueil » en gradient ; **Error** avec « ⚠️ » rouge (drop-shadow rouge) au-dessus d'un fond rouge sombre, h1 « Oups — quelque chose a coincé », paragraphe FR, boutons « Réessayer » (gradient) + « Retour à l'accueil » (ghost outline violet) ; **`<html lang="fr">`** au lieu de `lang="en"` ; **`theme-color`** `#7c3aed` (violet Kingstone, au lieu de `#8b5cf6` mauve Lovable) ; **`data-motion`** posé dynamiquement via `useMotionSettings().motionAttr` dans un `useEffect` au mount (préparation RT.B.7) ; toutes les classes shadcn (`bg-background`, `text-foreground`, `bg-primary`, etc.) supprimées au profit de styles inline cohérents avec l'identité visuelle. `reportLovableError` toujours appelé dans `ErrorComponent` (observabilité conservée) ; `router.invalidate() + reset()` conservés dans le bouton « Réessayer ». **N'a PAS été inclus** : `pendingComponent: SplashScreen` —) les classes `kg-splash`/`kg-splash-particle` référencées par `SplashScreen.tsx` n'existent pas encore dans `src/views/theme.css` (vérifié par `grep -c kg-splash src/views/theme.css` → 0) ; les inclure maintenant provoquerait un fallback CSS avec splash visuellement cassé. **Dette résiduelle** : ajout des classes `kg-splash*` dans `src/views/theme.css` + branchement `pendingComponent` prévu pour RT.B.7 (étape suivante). **Contexte** : avant RT.B.6, les composants NotFound/Error utilisaient encore entièrement le style shadcn/Lovable (classes CSS mortes depuis RT.B.2, mais le code les référençait toujours), avec textes en anglais. RT.B.6 applique la refonte identité Kingstone complète. **Vérifications brutes** : `grep -E "bg-background|text-foreground|bg-primary|text-primary-foreground|border-input|hover:bg-accent"` sur le nouveau fichier → **0 occurrence** ; `grep -E "html lang="` → `lang="fr"` ; `grep -E "theme-color"` → `content: KG_VIOLET` (résolu `#7c3aed`) ; `grep -c "7c3aed\\|22d3ee\\|KG_VIOLET\\|KG_CYAN\\|KG_GRADIENT\\|KG_BG"` → **10 références palette KG** ; `npx tsc --noEmit` → **0 erreur** (exit 0) ; `npx vite build` → **exit 0** (built in 2.34s, Nitro généré sans erreur). **Écarts par rapport au plan initial** : aucun sur le périmètre ; le `pendingComponent: SplashScreen` prévu au plan est **différé** à RT.B.7 pour la raison technique décrite ci-dessus. **Validation par Yannick (« ok »)**. Prochain déclenchement : RT.B.7 (branchement `pendingComponent: SplashScreen` + ajout classes `kg-splash*` dans `src/views/theme.css`, montage `PowerCutOverlay` dans `src/routes/_authenticated/app/route.tsx`, vérification early-return `LoadingScreen` dans `src/routes/index.tsx`).
| 30/07/2026 | **RT.B.7 — Branchement SplashScreen + PowerCutOverlay** — 4 classes CSS ajoutées en fin de `src/views/theme.css` : `.kg-splash` (full-viewport + radial-gradient violet sombre), `.kg-splash-particles` (calque absolu derrière le logo), `.kg-splash-particle` (positionnée `bottom: -30px`, animation `floatUp` réutilisée du `.bg-particle` existant), `.kg-splash-logo` (z-10, flex col, gap 16px). **Branchement SplashScreen** dans `src/routes/__root.tsx` : `import { SplashScreen } from "@/views/components/SplashScreen"` ajouté + `pendingComponent: SplashScreen` ajouté à la config `createRootRouteWithContext`. **Branchement PowerCutOverlay** dans `src/routes/_authenticated/app/route.tsx` : `import { PowerCutOverlay } from "@/views/components/PowerCutOverlay"` ajouté + `<PowerCutOverlay />` monté juste après `<ToastContainer />` dans le JSX (couvre toutes les routes `_authenticated/app/*` : caisse, salle, tickets, stats, reglages). **Contexte** : `SplashScreen.tsx` et `PowerCutOverlay.tsx` étaient tous deux définis sur disque (avec commentaires détaillés de l'étape 3C et 3D) mais **jamais branchés** dans l'arborescence runtime. `SplashScreen` était explicitement destiné au `pendingComponent` selon son propre commentaire de header. `PowerCutOverlay` était explicitement destiné à `app/route.tsx` selon son propre commentaire de header. Les deux étaient orphelins. Les classes `kg-splash*` n'existaient pas non plus dans `theme.css` (vérifié par `grep -c kg-splash src/views/theme.css` → 0 avant RT.B.7) —) donc même si on les avait branchés avant, ils auraient fait un fallback CSS invisible. RT.B.7 résout les deux dettes en même temps. **Vérifications brutes** : `grep -c kg-splash src/views/theme.css` → **4** ; `grep "pendingComponent" src/routes/__root.tsx` → `pendingComponent: SplashScreen` ; `grep "PowerCutOverlay" src/routes/_authenticated/app/route.tsx` → import + `<PowerCutOverlay />` ; `npx tsc --noEmit` → **0 erreur** (exit 0) ; `npx vite build` → **exit 0** (built in 1.74s, Nitro généré sans erreur). **Effet runtime attendu** : (a) au boot de l'app, TanStack Router rend `SplashScreen` (pending) avec logo + 8 emojis GameZone flottants, puis transite vers la route cible dès que les chunks sont prêts ; si `prefers-reduced-motion: reduce` est actif → SplashScreen rend le logo seul, sans particules. (b) Sur n'importe quelle page `/app/*`, `PowerCutOverlay` est monté en global ; si `useStore.powerCutActive === true`, l'overlay s'affiche par-dessus l'UI avec un bouton « Reprise manuelle » visible pour `platform_admin` / `lounge_admin`. **N'a PAS été touché** : `LoadingScreen.tsx` reste orphelin (commentaire de header dit « à harmoniser visuellement à l'étape 3 », qui n'est jamais arrivée) ; pas d'import runtime dans `src/routes/`, donc rien à casser. Traitement prévu pour une étape ultérieure si pertinent (sinon suppression pure et simple). **Validation par Yannick (« ok »)**. Prochain déclenchement : RT.B.8 (renommages hérités Lovable : `src/integrations/lovable/` → `src/integrations/cloud/`, `src/lib/lovable-error-reporting.ts` → `src/lib/error-reporting.ts`, mise à jour des imports dans tout le projet).
| 30/07/2026 | **RT.B.8 â€"
...
Cette section consigne les travaux du 03/08/2026 : audit global du projet (5 agents Explore), corrections des 11 dettes détectées (ND1-ND11), et **ouverture du CHANTIER 1 — Fondations**. À lire en premier pour reprendre le projet.

### Audit 2026-08-03

5 angles audités en parallèle : cartographie, sécurité, routes & navigation, code applicatif, conformité règles. 11 nouvelles dettes détectées (ND1-ND11). État codebase vérifié sur disque + Dashboard Supabase.

### Corrections exécutées 03/08/2026

| # | Fix | Description | Statut |
|---|---|---|---|
| Fix #1 | ND1 — XSS `document.write` | Helper `escapeHtml` appliqué dans `Caisse.tsx` + `Tickets.tsx` | ✅ |
| Fix #2 | Dette #1 — `.env` tracking | Vérification infirme l'audit initial — `.env` non tracké, A.1 a bien fonctionné | ✅ |
| Fix #3 | ND3 — `/client` sans `beforeLoad` | RBAC ajouté dans `client/route.tsx:15-33` | ✅ |
| Fix #4 | ND5 — Validation faible `/platform` | 7 helpers + bornes sur `createTenant`/`sendInvitation`/`addAdmin`/`saveTenantSettings` | ✅ |
| Fix #5 | ND4 — `requireSupabaseAuth` mort | Suppression YAGNI du fichier `auth-middleware.ts` (109 lignes) | ✅ |
| Fix #6 | ND6 + ND6bis + ND6ter | Casts `as any` ×5 supprimés + `src/hooks/` vide + dépendance `@lovable.dev/cloud-auth-js` retirée | ✅ |
| Fix #7 | ND8 — `handleImportFile` | 3 type guards (`isValidSession`/`isValidTicket`/`isValidPoste`) + whitelist `paymentMethod` | ✅ |
| Fix #8 | ND7 — `merge` Zustand | Type guards sur sessions/tickets/postes avant `setState` | ✅ |
| Fix #9 | ND2 — PII localStorage | **Phase A** : anonymisation 7j (alignée `TICKET_VALID_MS`). **Phase B** : chiffrement AES-GCM via Web Crypto API + clé IndexedDB. Honnêteté documentée : NE protège PAS contre XSS actif. | ✅ |
| Fix #10 | ND11 — Conflit lockfiles | `package-lock.json` supprimé, `pnpm-lock.yaml` conservé, `packageManager: pnpm@9` ajouté | ✅ |

**Vérifications cumulées** : `npx tsc --noEmit` exit 0 sur chaque Fix. `npx vite build` exit 0 sur chaque Fix. Aucune régression introduite.

### Fichiers créés / modifiés

- `SECURITY.md` (nouveau, racine projet) — modèle de menace, RBAC, anonymisation, limites HONNÊTES
- `src/lib/utils.ts` — ajout `escapeHtml`
- `src/lib-app/secure-storage.ts` (nouveau) — chiffrement PII AES-GCM
- `src/store/useStore.ts` — anonymisation 7j + chiffrement jour-J
- `src/views/screens/Caisse.tsx` — XSS + casts `as any`
- `src/views/screens/Tickets.tsx` — XSS
- `src/views/screens/Reglages.tsx` — validation import
- `src/routes/_authenticated/platform/index.tsx` — validation forms
- `src/routes/_authenticated/client/route.tsx` — beforeLoad RBAC
- `src/integrations/cloud/auth.ts` — nettoyage commentaire
- `src/integrations/supabase/auth-middleware.ts` — supprimé (YAGNI)
- `package.json` — packageManager + engines + retrait dépendance morte

### Dettes restantes (hors périmètre Claude / décisions externes)

| Dette | Action requise | Statut |
|---|---|---|
| #1 (rotation) | Rotation `SUPABASE_SERVICE_ROLE_KEY` côté Supabase Dashboard | Action Yannick |
| #4 (licences) | Système d'abonnement — chantier 3-4 sessions | Backlog |
| #5 (isolation RLS) | Décision produit phase C | À clarifier |
| #23 (`loyalty_points` UI) | Backlog | Backlog |
| #25 (`audit_log`) | Chantier dédié — traçabilité platform_admin | À planifier |

### CHANTIER 1 — Fondations : ouverture

**Date d'ouverture** : 03/08/2026 (ce même jour).

**Périmètre** : Tables DB + auth + formulaires + extension ticket. Hors scope : migration cookie httpOnly (dette #26, chantier dédié).

**Décisions actées avant codage** (cf. [[chantier1-statut]] + [[chantier1-couplages]]) :

1. `mailchecker` accepté comme dépendance (jeu de données, anti-jetable, côté serveur uniquement).
2. Table `staff_applications` séparée (option 2, tranchée à E1).
3. `is_minor` calculée sur `profiles`, PAS de CHECK dur (cohérent RT.H.5).
4. Ordre d'exécution A → B → C → D, **une seule migration SQL** regroupant tous les changements schéma.

**Sections** :
- **A** — Migration DB : `profiles` étendu + `reservations` + `consoles` + `staff_applications`
- **B** — Formulaire d'inscription client (anti-jetable, age, reset password, Google Sign-In déjà OK)
- **C** — Candidature staff + écran admin approbation
- **D** — Extension de ticket (bouton "⏱ Ajouter" lié réservation)
- **E** — Cookie httpOnly (HORS SCOPE, dette #26)

**Prochaine étape (Phase A.0)** : Lecture de l'état réel des fichiers + rédaction de la migration SQL consolidée. En attente validation "ok" pour démarrer.


---

## ?? Journal de la session 03/08 ? 12/08/2026 (documentée a posteriori le 12/08)

> **Contexte de rédaction** : à la reprise du 12/08, l''intégralité du travail réalisé entre le 03/08 et le 12/08 (fix ND1-ND11, CHANTIERS 1/2/3, durcissement RLS) était **présent sur disque mais jamais committé**, et `MEMOIRE_CAISSE.md` n''en gardait aucune trace (section 03/08 ouverte mais pas de suivi d''exécution). Ce journal comble ce trou de gouvernance — même pattern que l''historique RT.A.0 (interruption de session sans commit). Chaque entrée est constatée **sur disque** (fichiers, dates de modification, contenu) et validée par `npx tsc --noEmit` exit 0 + `npx vite build` exit 0 le 12/08 avant commit.

### ND.0 — Audit global 03/08/2026 (5 agents Explore)
**Constat** : 11 dettes ND1-ND11 identifiées (XSS `document.write`, PII en clair, `/client` sans guard RBAC, validation `/platform` faible, lockfiles en conflit, etc.). Détaillé dans la « Section 03/08/2026 » ci-dessus.

### ND.1-ND.11 — 11 fixes applicatifs (03/08/2026)
**Fichiers** : `SECURITY.md` (créé), `src/lib/utils.ts` (escapeHtml), `src/lib-app/secure-storage.ts` (créé, AES-GCM + IndexedDB), `src/store/useStore.ts` (chiffrement + anonymisation 7j + guards merge), `src/views/screens/{Caisse,Tickets,Reglages}.tsx`, `src/routes/_authenticated/platform/index.tsx` (validation forms), `src/routes/_authenticated/client/route.tsx` (beforeLoad), `src/integrations/cloud/auth.ts`, `src/integrations/supabase/auth-middleware.ts` (supprimé, YAGNI), `package.json` + `package-lock.json` (supprimé) + `pnpm-lock.yaml` (bascule pnpm).

### CH.1 — CHANTIER 1 Fondations (migration + UI, 03?10/08/2026)
- **Migration `20260803113500_6_chantier1_fondations.sql`** (18.5 Ko, 401 lignes) : `ALTER TYPE poste_status ADD VALUE 'reserved'`, extension `profiles` (nom, prenom, sexe, age, telephone, indicatif_pays `+242`, `is_minor` GENERATED), tables `reservations`, `consoles`, `staff_applications`, triggers, RLS.
- **Edge `check-email-jetable`** (07/08) : mailchecker npm, `verify_jwt=false` (config.toml).
- **`src/routes/auth.tsx`** : modes `signup-staff` (candidature avec dropdown tenants) + `reset` (mot de passe oublié), champs PII étendus (nom/prenom/indicatif/téléphone), fix BUG-2 `/auth?/` (getSession ? getUser).
- **`src/routes/_authenticated/platform/index.tsx`** : section approbation candidatures staff (join profiles + tenants via FKs `!*_fkey`), +829 lignes au total de la session.

### CH.3 — CHANTIER 3 Suppression de compte (08?10/08/2026)
- **Migration `20260808220000_7_delete_own_account.sql`** (22 Ko) : stratégie C hybride — soft-delete 30j (`deleted_at`, `scheduled_purge_at`), `banned_until` sur auth.users, purge pg_cron T+30j, fonctions `soft_delete_profile` / `restore_deleted_profile` / `purge_expired_profiles`, anti-footgun dernier platform_admin.
- **`20260809000003_8_drop_verification_functions.sql`** : rollback fonctions de vérification chantier (dettes de dev).
- **`20260810000000_9_*` + `20260810000001_9_*`** : FKs explicites `staff_applications.user_id` / `user_tenant_roles.user_id` ? `public.profiles(id)` ON DELETE RESTRICT (dette #27, joints PostgREST typables).
- **`20260810133905_seed_clients_fixtures_profile.sql`** : fixtures RTE legacy — création profiles manquants + rôle `client` (test FEAT-1).
- **Edge `delete-own-account` + `restore-account-admin`** (10/08) : `verify_jwt=true`, admin-only, rollback si échec banned_until.
- **`src/views/components/DoubleConfirmModal.tsx`** (créé) : confirmation à double saisie (« SUPPRIMER ») pour actions irréversibles.

### RLS 11/08 — Durcissement `tenants` côté anon (11/08/2026)
- `20260811040039` : REVOKE anon + GRANT colonnes spécifiques + policy « Public can list active tenants ».
- `20260811041738` : tentative `auth.uid() IS NOT NULL` — **insuffisante** (permissions évaluées avant court-circuit).
- `20260811050000` : recréation des 2 policies legacy avec `TO authenticated` explicite ? 42501 anon résolu. **Dette #32** : staff ? `billing_status` de sa salle reste ouvert (chantier à part).

### CH.2 — CHANTIER 2 Réservation en ligne + CinetPay (12/08/2026)
- **Migration `20260812000000_10_chantier2_paiement.sql`** (16.7 Ko, 343 lignes) : `ALTER TYPE payment_method ADD VALUE 'carte'`, RPC `compute_reservation_price` (SECURITY DEFINER, source de vérité du prix), table `payments`, `idempotency_keys`, indexes composites, RLS, CHECK durée = 30 min.
- **Edge `create-payment-intent`** (16.9 Ko) : recalcule le prix côté serveur, idempotence anti double-clic, CinetPay Airtel/MTN/carte.
- **`src/integrations/supabase/types.ts`** régénéré : PostgrestVersion 14.15, 14 tables, `payment_method` inclut `carte`.

### Écarts / points de vigilance (12/08/2026)
- **Push DB et déploiement edge NON exécutés** : aucune de ces migrations n''est appliquée en remote (projet `ybqsfyufnpmhlhdtwkon`) sauf les 5 poussées le 02/08. Action externe Yannick : `supabase db push` puis `supabase functions deploy`.
- **Références manquantes** : les migrations citent `memory/chantier1-statut.md`, `memory/chantier1-couplages.md`, `memory/chantier2-reservation-paiement.md`, `memory/chantier3-delete-account.md` — dossier `memory/` **absent** sur disque. Décisions accessibles uniquement dans les commentaires SQL.
