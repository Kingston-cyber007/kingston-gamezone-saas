...
import { fmtTime, fmtDuration, fmtMoney, fmtMs, isTicketValid, msToMin, generateTicketCode, TICKET_VALID_MS } from '../lib/helpers';
import { playAlertSound, playSessionStart, playTicketScan, speakWarning, speakExpired, playCustomSound, isSpeechSupported } from '../lib/audio';
import { showToast } from '../components/Toast';
import { fireWebNotification } from '../lib/notifications';
...
function ProgressRing({ remaining, total, warning, paused }: { remaining: number; total: number; warning: boolean; paused: boolean }) {
  ...
  const stroke = paused ? '#A0A8C0' : warning ? '#FF5C5C' : '#E8A33D';
  return (
    <svg width="132" height="132" viewBox="0 0 132 132" style={{ transform: 'rotate(-90deg)' }}>
      <circle fill="none" stroke="#2A2F3B" strokeWidth="8" cx="66" cy="66" r={r} />
...
export function SalleView() {
  const { postes, sessions, tickets, settings, updatePoste, addSession, updateTicket, todayKey } = useStore();
  ...
  function handleResumeCode() {
    ...
    if (!ticket) {
      showToast('Code ticket introuvable', '❌', '#FF5C5C');
      return;
    }
    if (!isTicketValid(ticket)) {
      showToast('Ticket expiré — reprise impossible', '⏰', '#FF5C5C');
      return;
    }
    if (!ticket.savedRemainingMs || ticket.savedRemainingMs <= 0) {
      showToast('Pas de temps restant sur ce ticket', '⚠️', '#E8A33D');
      return;
    }
    if (ticket.usedSavedTime) {
      showToast('Temps restant déjà utilisé', '⚠️', '#E8A33D');
      return;
    }
  ...
  function powerCutAll() {
    ...
    showToast(`${busy.length} session(s) gelées — temps sauvegardés`, '⚡', '#E8A33D');
  }
  ...
  return (
    <div className="salle-view">
      <div className="section-head flex items-baseline justify-between mb-5 flex-wrap gap-3">
        ...
        <button className="power-cut-btn" onClick={powerCutAll} title="Geler toutes les sessions (coupure électricité)">
          ⚡ Coupure
        </button>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="ring-wrap">
                ...
                {!isBusy && (
                  <svg width="132" height="132" viewBox="0 0 132 132" style={{ transform: 'rotate(-90deg)' }}>
                    <circle fill="none" stroke="#2A2F3B" strokeWidth="8" cx="66" cy="66" r="54" />
                  </svg>
                )}
...
function SessionModal({ posteId, mode, resumeMs, resumeTicketId, onClose, onConfirm }: {
  ...
  function handleConfirm() {
    if (!canConfirm()) {
      if (!selectedTicketId) showToast('Sélectionne un ticket client', '⚠️', '#E8A33D');
      else if (finalDuration < 30) showToast('Durée minimum 30 min', '⚠️', '#E8A33D');
      else showToast('Sélectionne une durée', '⚠️', '#E8A33D');
      return;
    }
    onConfirm({
          ...
          {isResume && resumeTicket ? (
            ...
            <>
              ...
              {validTickets.length === 0 && (
                <p className="field-hint" style={{ color: '#FF5C5C' }}>⚠️ Aucun ticket valide — crée d'abord un ticket dans l'onglet Tickets</p>
              )}
            </>
          )}
        </div>
        ...
        {!isResume && (
          <div className="field-section">
            ...
            {customMin && parseInt(customMin) > 0 && parseInt(customMin) < 30 && (
              <p className="field-hint" style={{ color: '#FF5C5C' }}>⚠️ Minimum 30 minutes pour une nouvelle session</p>
            )}
          </div>
        )}
...
function ExtendModal({ posteId, onClose, onConfirm }: {
  ...
  function handleConfirm() {
    ...
    if (chosen < 30 && !isRemainder) {
      showToast('Min 30 min sauf si c\'est le reliquat restant', '⚠️', '#E8A33D');
      return;
    }
    onConfirm(chosen);
  }
    ...
    <div className="modal-overlay active">
      <div className="modal">
        ...
        {isUnder30 && !isRemainder && (
          <p className="field-hint" style={{ color: '#FF5C5C' }}>⚠️ Minimum 30 min sauf pour le reliquat ticket</p>
        )}
